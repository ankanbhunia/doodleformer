...
class GraphFunc(nn.Module):
    ...
    def forward(self, graph_input_raw, graph_label, bbox):
        ...
        Aij = F.softmax(self.L(BoxRelationalEmbedding(bbox, trignometric_embedding = False))).squeeze(-1)
...
class context_aware_generator(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        self.label_embedding = nn.Embedding(num_classes, 180)
        #self.cond_encoder = ResnetEncoder128()
        num_w = 128 + 180
    ...
    def forward(self, z, bbox, z_im=None, y=None):
        ...
        #z_cond = self.cond_encoder(cond_raster)
        # 4x4
        x = self.fc(z_im).view(b, -1, 4, 4)
