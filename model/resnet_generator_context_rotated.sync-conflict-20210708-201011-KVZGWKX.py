...
def BoxRelationalEmbedding(f_g, dim_g=64, wave_len=1000, trignometric_embedding=True):
    ...
    x_min, y_min, x_max, y_max, theta = torch.chunk(f_g, 5, dim=-1)

    cx, cy, w, h, theta = torch.chunk(f_g, 5, dim=-1)
    ...
    delta_theta = torch.log((theta / theta.view(batch_size, 1, -1)))
    ...
    delta_theta = delta_theta.view(batch_size, matrix_size[1], matrix_size[2], 1)

    position_mat = torch.cat((delta_x, delta_y, delta_w, delta_h, delta_theta), -1)
