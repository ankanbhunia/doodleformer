...
def masks_to_layout(boxes, masks, H, W=None):
    ...
    if W is None:
        W = H
    boxes_ = boxes.view(b*num_o, -1)
    grid = _boxes_to_grid(boxes_, H, W).float().cuda(device=masks.device)
    img_in = masks.float().view(b*num_o, 1, M, M)
    sampled = F.grid_sample(img_in, grid, mode='bilinear')

    M = tgm.get_rotation_matrix2d((boxes_[:,:2]*2 - 1).to(sampled.device),( -boxes_[:,-1]*90).to(sampled.device), torch.ones(boxes_.shape[0]).to(sampled.device))
    affine_grid = F.affine_grid(M, sampled.size())
    sampled_ = F.grid_sample(sampled, affine_grid)

    #cv2.imwrite('im.png', (sampled[41,0]+sampled_[41,0]).detach().cpu().numpy()*255)

    #M = tgm.get_rotation_matrix2d(torch.tensor([[-1+0.6567*2, -1+0.2985*2]]), torch.tensor([-50.3895]), torch.ones(1))
    #affine_grid = F.affine_grid(M, sampled[41:42].size())
    #sampled_ = F.grid_sample(sampled[41:42], affine_grid.to('cuda'))
    #cv2.imwrite('im.png', (sampled[41,0]+sampled_[0,0]).detach().cpu().numpy()*255)


    #M = torch.tensor([[1, 0, -0.6567],[0, 1, 0.2985*2]]).unsqueeze(0)
    #affine_grid = F.affine_grid(M, sampled[41:42].size())
    #sampled_ = F.grid_sample(sampled[41:42], affine_grid.to('cuda'))
    #cv2.imwrite('im.png', (sampled[41,0]+sampled_[0,0]).detach().cpu().numpy()*255)
    ...
    return sampled_.view(b, num_o, H, W)
